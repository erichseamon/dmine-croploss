...
#Loading Agricultural Data
    ...
    sumcount1 <-  read.csv("PNW_summary_counts.csv")
    ...
    meanloss1 <- read.csv("PNW_summary_meanloss.csv")
    ...
    sumloss1 <- read.csv("PNW_summary_sumloss.csv")
    ...
    sumacres1 <- read.csv("PNW_summary_sumacres.csv")
    ...
    setwd("/dmine/data/USDA/agmesh-scenarios/Allstates/summaries/")
    sumall1 <- read.csv("PNW_summary_all.csv")
    sumall2 <- subset(sumall1, state == state1)
    sumall2 <- subset(sumall2, county == county1)
    sumall2 <- subset(sumall2, commodity == commodity1)
    ...
    #--by county
    claimaggall_aggy_trueloss <- aggregate(loss ~ year + county + damagecause + state + commodity, claimaggall_final2, sum)
    
    claimaggall_aggy_loss <- aggregate(cube_loss ~ year + county + damagecause + state + commodity, claimaggall_final2, sum)
    claimaggall_aggy_acres <- aggregate(cube_acres ~ year + county + damagecause + state + commodity, claimaggall_final2, sum)
    claimaggall_aggy_acresloss <- aggregate(acres ~ year + county + damagecause + state + commodity, claimaggall_final2, sum)
    
    claimaggall_aggy_lossacres <- aggregate(cube_loss/cube_acres ~ year + county + damagecause + state + commodity, claimaggall_final2, sum)
    claimaggall_aggy_lossperacre <- aggregate(loss/acres ~ year + county + damagecause + state + commodity, claimaggall_final2, sum)
    ...
    cif <- merge(combined_idaho.df, countylist, by = "countyfips")
    cif$month <- trimws(cif$month)
    ...
    gridmetmonthly <- data.frame(combined.df)
    ...
    gridmetmonthly$county <- trimws(gridmetmonthly$county)
    gridmetmonthly$month <- trimws(gridmetmonthly$month)
    #gridmetmonthly$commodity <- trimws(gridmetmonthly$commodity)
    #gridmetmonthly$damagecause <- trimws(gridmetmonthly$damagecause)

    #gridmetmonthly <- subset(gridmetmonthly, state == statez1a)
    gridmetmonthly <- subset(gridmetmonthly, county == county1)
    ...
    #gridmetmonthly$monthchar <- factor(gridmetmonthly$monthchar, month.abb, ordered=TRUE)
    #gridmetmonthly$monthchar <- as.numeric(gridmetmonthly$monthchar)
    
    gridmetmonthly$yearmon <- as.yearmon(paste(gridmetmonthly$month, gridmetmonthly$year))
    gridmetmonthly <- gridmetmonthly[order(gridmetmonthly$yearmon), ]
    #gridmetmonthly <- gridmetmonthly[order(gridmetmonthly[,18], gridmetmonthly[,21]), month.abb, ordered=TRUE]
    ...
    gridmetmonthly$month <- tolower(gridmetmonthly$month)
    ...
    ymll <- c(2002:2015)
     ...
     yearmatchlist <- c(2001,2002,2002,2003,2003,2004,2004,2005,2005,2006,2006,2007,2007,2008,2008,2009,2009,2010,2010,2011,2011,2012,2012,2013,2013,2014,2014,2015,2015)
     ...
     colnames(yml)  <- "yearmatch"
      ...
      #a file is created for each 
       ...
       for (p in climmonth){  
         ...
         p3 <- cbind(p2, num2)         
         colnames(p3)[24] <- "yearmatch"
         p3_noprecip <- p3[, -c(3)]
         p3_precip <- as.data.frame(p3[, c(3, 24)])
         colnames(p3_precip) <- c("pr", "yearmatch")
         
         p4_precip <- aggregate(p3_precip[, 1], list(p3_precip$yearmatch), sum)
         colnames(p4_precip) <- c("year", "pr")
         p4 <- aggregate(p3_noprecip[, 2:15], list(p3_noprecip$yearmatch), mean)
         p4 <- cbind(p4, p4_precip$pr)
         colnames(p4)[16] <- c("pr")
         setwd("/dmine/data/USDA/agmesh-scenarios/Allstates/climatematrix_all_Idaho_sum_precip/")
         colnames(p4)[1] <- "year"
         ...
         write.csv(claimaggall_aggy_loss, file = paste(state1, "_", county1, "_", commodity1, "_", damage1, "_cube_root_loss.csv", sep=""))
         
         write.csv(claimaggall_aggy_acres, file = paste(state1, "_", county1, "_", commodity1, "_", damage1, "_cube_root_acres.csv", sep=""))
         
         write.csv(claimaggall_aggy_trueloss, file = paste(state1, "_", county1, "_", commodity1, "_", damage1, "_loss.csv", sep=""))
         
         write.csv(claimaggall_aggy_lossperacre, file = paste(state1, "_", county1, "_", commodity1, "_", damage1, "_loss_per_acre.csv", sep=""))
         
         write.csv(claimaggall_aggy_acresloss, file = paste(state1, "_", county1, "_", commodity1, "_", damage1, "_acres_loss", sep=""))
         
         
         write.csv(claimaggloss_aggy2, file = paste(state1, "_", county1, "_", commodity1, "_", damage1, "_loss_month.csv", sep=""))
         write.csv(claimaggall_aggy_lossacres, file = paste(state1, "_", county1, "_", commodity1, "_", damage1, "_cube_root_loss_per_acre.csv", sep=""))
         
         
         write.csv(claimaggall_aggy_lossacres_year, file = paste(state1, "_", county1, "_", commodity1, "_", damage1, "_cube_root_acres.csv", sep=""))
...
setwd("/dmine/data/USDA/agmesh-scenarios/Allstates/summaries/")
sumloss1 <- read.csv("PNW_summary_sumloss.csv")
sumloss1 <- subset(sumloss1, county == "Minidoka" | county == "Twin Falls" | county == "Fremont" | county == "Madison" | county == "Bingham" | county == "Power" | county == "Cassia" | county == "Bonneville" | county == "Bannock" | county == "Teton" | county == "Oneida" | county == "Franklin" | county == "Caribou" | county == "Bear Lake")
county11 <- unique(sumloss1$county)


for (i in county11) {

  climate_cropcombo("ID", i, "WHEAT", "Drought") 
